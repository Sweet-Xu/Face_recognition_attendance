...
public class AttendItemController extends BaseController {
    ...
    public void receiveAttendItem(CheckRecordInputDto inputDto) {
        try {
            ...
            for (Attend t : attends) {
                ...
                String courseStartTimeString = stringBuffer.replace(11, 19, attendRule.getCourseStartTime()).toString();
                String courseEndTimeString = stringBuffer.replace(11, 19, attendRule.getCourseEndTime()).toString();
                String checkStartTimeString = stringBuffer.replace(11, 19, attendRule.getCheckStartTime()).toString();
                String checkEndTimeString = stringBuffer.replace(11, 19, attendRule.getCheckEndTime()).toString();
                ...
                //课程结束时间
                Long courseEndTimeSecond = courseEndTimeDate.getTime();
                ...
                if (checkStartTimeSecond <= checkTimeSecond && checkTimeSecond < compareSecond) {
                    ...
                    for (AttendItem a : signInAttendItems) {
                        if (a.getAttendResult().equals("暂无记录")) {
                            ...
                            this.attendItemService.updateAttendItem(a);
                        }
                    }
                }
                ...
                if (compareSecond <= checkTimeSecond && checkTimeSecond <= checkEndTimeSecond) {
                    ...
                    for (AttendItem b : signOutAttendItems) {
                        if (b.getAttendResult().equals("暂无记录")) {
                            ...
                            this.attendItemService.updateAttendItem(b);
                        }
                    }
                }
            }
    ...
    @ResponseBody
    public Map<String,Object> attendDetail(Long attendId){
        System.out.println("收到的考勤表id:"+attendId);
       return this.attendItemService.findAllAttendDetail(attendId);
    }
    ...
    @GetMapping("/api/attendItem/excel")
    @ResponseBody
    public ResponseBo attendItemExcel(AttendItem attendItem){
        try{
            List<AttendResultExport> list = this.attendItemService.exportAttendAllResult(attendItem);
            return FileUtil.createExcelByPOIKit("考勤结果汇总表",list, AttendResultExport.class);
        }catch (Exception e){
    ...
    public ResponseBo attendItemCsv(AttendItem attendItem) {
        try {
            ...
            return FileUtil.createCsv("考勤结果汇总表", list, AttendItem.class);
        } catch (Exception e) {
    ...
    @GetMapping("api/attendItem/getAttendDynamics")
    @ResponseBody
    public List<AttendDynamicsOutputDto> getAttendDynamics(){
        return this.attendItemService.getAttendDynamics();
    }
}
